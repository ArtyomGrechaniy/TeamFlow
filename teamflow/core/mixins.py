...
class UserTeamObjectQuerysetMixin:
...
class OwnershipMixin:
    """Определяет принадлежность объекта к пользователю или команде."""
    def get_team_id(self):
        return self.kwargs.get("team_id")
...
class TeamAccessMixin(LoginRequiredMixin, UserPassesTestMixin):
    """
    Предоставляет доступ к действиям в команде только
    ...
    def test_func(self):
        if self.get_team_id():
            return self.has_team_access(
                self.request.user, self.get_team_id(), self.required_roles
            )
        else:
            return True
...
class FormContextMixin:
    """
    Добавляет общий контекст для страниц создания и редактирования объектов.

    Во view-классе необходимо задать 'form_type' который соответствует одному из ключей.
    """
    form_type = None
        ...
        if team_id:
            if self.form_type == "task":
                cancel_url = reverse_lazy(
                    "teams:tasks:task_list", kwargs={"team_id": team_id}
                )
            elif self.form_type == "tasks_category":
                cancel_url = reverse_lazy(
                    "teams:tasks:category_list", kwargs={"team_id": team_id}
                )
            elif self.form_type == "expense":
                cancel_url = reverse_lazy(
                    "teams:expenses:expense_list", kwargs={"team_id": team_id}
                )
        ...
        context.update(
            {
                ...
                "submit_text": (
                    config["create_submit"] if is_create else config["edit_submit"]
                ),
    ...
    def get_success_url(self):
        ...
        if self.form_type == "task":
            ...
            return reverse_lazy("tasks:category_list")
...
class DeleteContextMixin:
    """
    Добавляет общий контекст для страниц подтверждения удаления объектов.

    Во view-классе необходимо задать 'delete_type' который соответствует одному из ключей.
    """
    delete_type = None
            ...
            context.update(
                {
                    ...
                    "icon": "💸",
                    "cancel_url": (
                        reverse_lazy("expenses:expense_list")
                        if not team_id
                        else reverse_lazy(
                            "teams:expenses:expense_list", kwargs={"team_id": team_id}
                        )
            ...
            context.update(
                {
                    ...
                    "icon": "🏷️",
                    "cancel_url": (
                        reverse_lazy("expenses:category_list")
                        if not team_id
                        else reverse_lazy(
                            "teams:expenses:category_list", kwargs={"team_id": team_id}
                        )
            ...
            context.update(
                {
                    ...
                    "icon": "🏷️",
                    "cancel_url": (
                        reverse_lazy("tasks:category_list")
                        if not team_id
                        else reverse_lazy(
                            "teams:tasks:category_list", kwargs={"team_id": team_id}
                        )
    ...
    def get_success_url(self):
        ...
        if self.delete_type == "expense":
            return (
                reverse_lazy("teams:expenses:expense_list", kwargs={"team_id": team_id})
                if team_id
                else reverse_lazy("expenses:expense_list")
            )

        elif self.delete_type == "task":
            return (
                reverse_lazy("teams:tasks:task_list", kwargs={"team_id": team_id})
                if team_id
                else reverse_lazy("tasks:task_list")
            )
            ...
            return (
                ...
                if team_id
                else reverse_lazy("expenses:category_list")
            )

        elif self.delete_type == "task_category":
            return (
                reverse_lazy("teams:tasks:category_list", kwargs={"team_id": team_id})
                if team_id
                else reverse_lazy("tasks:category_list")
            )
